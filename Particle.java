...
//        System.out.printf("Interpolated Velocity = %.4f %.4f\n",velocity[0],velocity[1]);
    ...
    /**
     * As above method but rather than a particular depth layer, reads in array of nearest two depth layers and distances to those depths.
     * This allows vertical interpolation of velocity to the particle's current depth.
     * Note that this still includes h
     * 
     * @param nrList
     * @param tt
     * @param u
    ...
    public static double[] velocityFromNearestListDepthInterp(double[][] nrList, int tt, float u[][][], float v[][][], double[][] depLayers)
    {
        double[] velocity = new double[2];
        double[] weights = new double[nrList.length];
        double usum1=0,vsum1=0,usum2=0,vsum2=0,sum1=0,sum2=0;
        for (int i = 0; i < nrList.length; i++)
        {
            if (nrList[i][1]!=0)
            {
...
//                tt,depLayer,(int)nrList[i][0],nrList[i][1],weights[i],u[tt][depLayer][(int)nrList[i][0]],v[tt][depLayer][(int)nrList[i][0]]);
//            System.out.println("velocityFromNearestList: "+i+" "+tt+" "+depLayer+" "+(int)nrList[i][0]);
            usum1=usum1+weights[i]*u[tt][(int)depLayers[0][0]][(int)nrList[i][0]];
            usum2=usum2+weights[i]*u[tt][(int)depLayers[1][0]][(int)nrList[i][0]];
            vsum1=vsum1+weights[i]*v[tt][(int)depLayers[0][0]][(int)nrList[i][0]];
            vsum2=vsum2+weights[i]*v[tt][(int)depLayers[1][0]][(int)nrList[i][0]];
...
//        System.out.printf("Interpolated Velocity = %.4f %.4f\n",velocity[0],velocity[1]);
    ...
    /**
     * double[] xy, int elemPart0, int meshPart,
     *       List<Mesh> meshes, String coordRef
     *
     */
    public static double[][] depthLayerNrList(double[] xy, double depth, int elemPart, int meshPart, List<Mesh> meshes, float[][] surfaceElevation, int tt)
    {
        double[][] layerList = new double[2][3];
        ...
        float[] meshNodeDepth = meshes.get(meshPart).getDepthNodexy();
        float[] meshElemDepth = meshes.get(meshPart).getDepthUvnode();
        float[] siglay = meshes.get(meshPart).getSiglay();
        
        int elem[] = findContainingElement(xy, elemPart, nodexy, trinodes, neighbours, false);
        int thisElem = elem[0];
        
        float localDepth = meshElemDepth[thisElem];
        
        boolean interpHoriz = true;
        float localDepth2;
        if (interpHoriz)
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                int nodeId = trinodes[i][thisElem];
                cornerDists[i] = (float)distanceEuclid2(xy[0],xy[1], (double)nodexy[0][nodeId], (double)nodexy[1][nodeId], meshes.get(meshPart).getCoordRef());
                cornerWeights[i] = (float)1.0/cornerDists[i];
                cornerVals[i] = meshNodeDepth[nodeId];
            }
            localDepth = (cornerWeights[0]*cornerVals[0]+cornerWeights[1]*cornerVals[1]+cornerWeights[2]*cornerVals[2])/
                    (cornerWeights[0]+cornerWeights[1]+cornerWeights[2]);
            float localElev = surfaceElevation[tt][trinodes[0][thisElem]];
            localDepth = localDepth + localElev;
            boolean blah = true;
        }
        
        float[] localLayerDepths = new float[siglay.length];
        for (int i = 0; i < localLayerDepths.length; i++)
        {
            localLayerDepths[i] = siglay[i]*localDepth;
        }
 
        double[][] layerListAll = new double[siglay.length][3];
        for (int i = 0; i < siglay.length; i++)
        {
            // NOTE: changed particle depth to be positive
            layerListAll[i][0] = (double)i;
            layerListAll[i][1] = depth - localDepth*siglay[i];
            layerListAll[i][2] = Math.abs(depth - localDepth*siglay[i]);
        }
        
        Arrays.sort(layerListAll, Comparator.comparingDouble(o -> o[2]));
        
        for (int i = 0; i < 2; i++)
        {
            layerList[i][0] = layerListAll[i][0];
            layerList[i][1] = layerListAll[i][1];
            layerList[i][2] = layerListAll[i][2];
        }
...
//                        }
        ...
        c[0]=whereami;
//        System.out.printf("findContainingElement %d %d %d %d %d %d %d\n",elemPart,c[0],c[1],c[2],c[3],c[4],c[5]);
//        if (c[0] == 0 || c[0] == -1)
//        {
...
//            nodexy, trinodes, neighbours);
        ...
        // Catch cases where 0 is listed as a neighbour due to having an empty "0" value in the FVCOM mesh data - and set the distance to be huge
        for (int i = 0; i < nrList.length; i++){
            if (nrList[i][0] == 0)
            {
                if (nrList[i][1] > 5000)
...
//        
            ...
            int stepsPerStep, String coordRef, boolean vertInterp)   // info on simulation length
    {
        ...
        {     
            ...
            // Exit if the particle is not in the domain
            if (this.nrList[0][0] == -1)
            {
                return advectStep;
            }
...
//                System.out.println(this.printLocation()+" "+this.getDepth()+" "+this.getElem()+" "+this.startSiteID);
...
//                st,advectStep[0],advectStep[1],vel[0],vel[1],dt,vel[0]*dt,vel[1]*dt);
     ...
     * @return 
     */
    public static double[] stepAhead(double[] xy, int elemPart, int depLayer, double timeStepAhead,
